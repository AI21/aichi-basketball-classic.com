...
function value_check($POST){
	...
	//hiddenのmatchng="name+name2,hoge1+hoge2"
	if(isset($POST["matchng"]) && $POST["matchng"] !== ""){
		$matchngArr = explode(',',$POST["matchng"]);
		if(is_array($matchngArr)){
			foreach($matchngArr as $strName){
				$nameAry = explode('+', $strName);
				if($formitem[$nameAry[0]] === $POST[$nameAry[1]]){
					$formitem[$nameAry[0]]=convert_encode('<strong class="error">同じ項目を選択しています。</strong>');
					$error++;
				}
			}
		}
...
//		mail(MAILTO, $mailsubject, $message, $mailheader);
	...
	if(strpos(PHPVER,'5') === false){
		...
		$replymessage = str_replace("\r","",str_replace("<br />","\n", $replymessage));

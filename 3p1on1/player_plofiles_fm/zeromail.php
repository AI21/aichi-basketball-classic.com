...
//			print '12345'.nl2br(print_r($formitem,1)); exit;
...
//print $error." = err1<br>";
	if(isset($_FILES)&&count($_FILES)!=0){
		...
		$formitem['tmpfile'] = $_SESSION['tmpfile'];
	}
//print $name[0]." = err2<br>";
//
//print nl2br(print_r($formitem,1));
	$formitem['Err'] = $error;
...
//				$_SESSION[$name] = convert_encode('<strong class="error">'.$label."[".$filename."] のファイル形式が不適切です。</strong>");
				$_SESSION[$name] = convert_encode('<strong class="error">'."ファイル形式をjpegにしてください。</strong>");
				$err = 1;
				break;
			}
...
//				$_SESSION[$name] = convert_encode($filename." (".($filesize/1000)."kb)".' <a href="'.UPLOADPASS.$filename.'"'.$target.' class="zmPreview">ファイルの確認</a>');
				$_SESSION[$name] = convert_encode($filename.'<br><img src="' . UPLOADPASS.$filename . '">'.' <a href="'.UPLOADPASS.$filename.'"'.$target.' class="zmPreview">ファイルの確認</a>');
	...
	if ($intFileCnt == 0 && $err == 0) {
		$_SESSION[$name] = convert_encode("<strong class=\"error\">画像を添付してください。</strong>");
		$err = 1;
//		$err = convert_encode("<strong class=\"error\">画像を添付してください。</strong>");
	}
//	print $intFileCnt." = PICCNT<br>";
	if(isset($err)){
		return $err;
	}
...
function zeromailAjax_send($formitem)
{
	...
	$mailsubject = MAILSUBJECT." : ".date("n/d h:i");
...
//		$mailfrom2 = get_mailfrom($name, $formitem['email']);
		$mailfrom2 = get_mailfrom($name, MAILFROM);

		$mailheader = "From: " . FROMNAME . "<" . $mailfrom2.">\r\n";
		if(BCC != "") $mailheader.="Bcc: ".BCC."\r\n";
		$mailheader .= "X-Mailer: ".SCRIPT."(Version ".VERSION.")\r\n";
...
//	$ext  = array(".gif",".png",".jpg",".bmp",".pdf",".swf",".txt",".xls",".doc",".ppt",".zip",".lzh",".tar.gz");
	$minetype = array("image/jpeg","image/pjpeg");
	$ext  = array(".jpg",".jpeg");

...
<!--										<small>登録内容の控えメールが必要な場合は、ご入力ください</small>-->
								...
								<tr<?= ($i % 2 == 1) ? ' class="bgw"' : ''; ?>>
									<th rowspan="2" class="players">選手<?= $i; ?><?php if($i <= 5): ?> <small class="alert">※</small><?php endif; ?></th>
									<th>背番号 / 氏名(ヨミカナ)</th>
									<td>
										No. <input name="number<?= $i; ?><?php if($i <= 5): ?>_req<?php endif; ?>_num" type="number" size="6" maxlength="3" min="0" max="99">
									</td>
									<td>
										氏名：<input name="name<?= $i; ?><?php if($i <= 5): ?>_req<?php endif; ?>" type="text" size="40"><br>
										カナ：<input name="kana<?= $i; ?><?php if($i <= 5): ?>_req<?php endif; ?>_jpk" type="text" size="40"placeholder="カタカナのみ">
									</td>
								</tr>
								<tr<?= ($i % 2 == 1) ? ' class="bgw"' : ''; ?>>
									<th>身長 / ポジション</th>
									<td colspan="2">
										<input name="tall<?= $i; ?><?php if($i <= 5): ?>_req<?php endif; ?>_num" type="number" size="6" maxlength="3" min="140" max="230"> cm&nbsp;/&nbsp;
								...
								<tr class="submitArea">
									<td colspan="4">
										...
										<?php endfor; ?>
										<?php if ( $blnSetTeam == TRUE ) : ?>
										<input type="hidden" name="require" value="member1,member2,member3,member4,member5,pos1,pos2,pos3,pos4,pos5">
										<?php else : ?>
										<input type="hidden" name="require" value="member1,member2,member3,member4,member5,pos1,pos2,pos3,pos4,pos5">
										<?php endif; ?>
